...
void Position::StaticInitialize()
{
	...
	for (int i = 0; i < 2; i++)
		for (int j = 0; j < 8; j++)
			for (int k = 0; k < 64; k++)
				Position::Zobrist[i][j][k] = r();

	for (int i = 0; i < 64; i++)
	{
		Position::ZobristEP[i] = r();
	...
	for (int i = 0; i < 16; i++)
		Position::ZobristCastle[i] = r();

	Position::ZobristToMove = r();
}
...
std::string Position::GetFen() const
{
	...
	// ToMove
	if (color == WHITE) result += " w"; else result += " b";
